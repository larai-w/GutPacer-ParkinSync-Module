...
**訂正(2026-07-22・実機H-4で判明)**: API側のIDトークン検証に使うチャネルIDは、**環境ごとの LINEログインチャネルID = LIFF ID の先頭数字**(開発 `2010720966` / 審査 `2010720967` / 本番 `2010720968`)。トークンの `aud` はこの値になる。以前ここに書いていた `2010428233` での検証は**誤り**で、実機では401になった。`2010428233` は Mini App/プロバイダ側のIDで、IDトークン検証には**使わない**。LINE環境で認証に失敗した場合、PIN画面へフォールバックせずエラー表示にして認証方式を混在させない。
