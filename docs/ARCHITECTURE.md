...
GutPacer は、家族介護者が排便・服薬・施設滞在の情報を記録し、設定された世帯でLINEの記録確認通知を受け取るためのWebアプリである。特定の疾患に限定せず、診断や治療方針を提供せず、記録と振り返りを支援する。
...
以下は本番提供中の構成ではない。将来複数世帯へ対応する場合は、ログインしたLINEユーザーIDをすべてのデータアクセスの境界にする。認証とデータ分離を先に実装し、その後に薬・便の選択肢・通知しきい値をユーザープロファイルから描画する。
